...
namespace NBT
{
		...
		public override string ToString ()
		{
			switch(Type) {
				...
				case TagType.ByteArray:
					return "TAG_Byte_Array" + NameString(Name) + ": [" + ((byte[]) Payload).Length + " bytes]";
				case TagType.String:
	...
	{
		static short ntoh(short x) {
			return IPAddress.NetworkToHostOrder(x);
		}
		static int ntoh(int x) {
			return IPAddress.NetworkToHostOrder(x);
		}
		static long ntoh(long x) {
			return IPAddress.NetworkToHostOrder(x);
		}
		...
		static BinaryTag ReadNamedTag(Stream ByteStream)
		{
	...
	{
		static short hton(short x) {
			return IPAddress.HostToNetworkOrder(x);
		}
		static int hton(int x) {
			return IPAddress.HostToNetworkOrder(x);
		}
		static long hton(long x) {
			return IPAddress.HostToNetworkOrder(x);
		}
